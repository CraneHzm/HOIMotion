...
class Player_Skeleton:
    def __init__(self, fps=30.0, show_images = False, objects_num=10):
        """ init function
    ...
    def play_xyz(self, path, pose_xyz, gaze, head, objects):
...
if __name__ == "__main__":
    data_path = '/scratch/hu/pose_forecast/adt_hoimotion/test/Apartment_release_work_skeleton_seq132_'
    
    gaze_path = data_path + 'gaze.npy'
    head_path = data_path + 'head.npy'
    pose_path = data_path + 'pose_xyz.npy'
    objects_dynamic_path = data_path + 'objects_dynamic.npy'
    objects_static_path = data_path + 'objects_static.npy'
    ...
    objects_dynamic = np.load(objects_dynamic_path)
    objects_static = np.load(objects_static_path)
    objects = np.concatenate((objects_dynamic, objects_static), axis=1)
    print("Objects shape: {}".format(objects.shape))

    player = Player_Skeleton(show_images = show_images, objects_num = objects.shape[1])
    player.play_xyz(data_path, pose, gaze, head, objects)