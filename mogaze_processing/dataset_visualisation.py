...
class Player_Skeleton:
    ...
    def play_xyz(self, pose_xyz, gaze, head_direction):      
        ...
        gaze_pos = pose_xyz[:, 4*3:5*3] + gaze*0.5
        head_pos = pose_xyz[:, 4*3:5*3] + head_direction*0.5
    ...
    # play the sequence of human pose and pickable objects in xyz representations
    def play_xyz_objects(self, pose_xyz, gaze, head_direction, objects):      
        # gaze position = head position + gaze direction
        gaze_pos = pose_xyz[:, 4*3:5*3] + gaze*0.5
        head_pos = pose_xyz[:, 4*3:5*3] + head_direction*0.5
        pose_xyz = np.concatenate((pose_xyz, gaze_pos), axis = 1)
        ...
        object_num = objects.shape[1]//3
        self._objects = []
        colors = ['#1ffe01', '#34cb34', '#649b70', '#649b70', '#7e6d92']
        for j in range(object_num):
            #self._objects.append(self._ax.scatter(objects[0, j*3], objects[0, j*3+1], objects[0, j*3+2], c=colors[j], s = 30))
            self._objects.append(self._ax.plot([objects[0, j*3], objects[0, j*3]+0.01], [objects[0, j*3+1], objects[0, j*3+1]+0.01], [objects[0, j*3+2], objects[0, j*3+2]+0.01], lw=2, c=colors[j]))
        ...
        for i in range(pose_xyz.shape[0]):
            ...
            for j in range(object_num):
                x = np.array([objects[i, j*3], objects[i, j*3]+0.01])
                y = np.array([objects[i, j*3+1], objects[i, j*3+1]+0.01])
                z = np.array([objects[i, j*3+2], objects[i, j*3+2]+0.01])                
                self._objects[j][0].set_xdata(x)
                self._objects[j][0].set_ydata(y)                               
                self._objects[j][0].set_3d_properties(z)
...
if __name__ == "__main__":
    data_path = "/scratch/hu/pose_forecast/mogaze_hoimotion/p1_1/pick_010_"
    pose_xyz_data_path = data_path + "pose_xyz.npy"
    gaze_data_path = data_path + "gaze.npy"
    head_data_path = data_path + "head.npy"   
    objects_data_path = data_path + "objects_dynamic.npy"
    ...
    # xyz positions of the pickable objects
    objects = np.load(objects_data_path)
    print("Objects shape: {}".format(objects.shape))
    
    player = Player_Skeleton()
    player.play_xyz_objects(pose_xyz, gaze, head, objects)